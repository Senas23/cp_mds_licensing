#!/usr/bin/python3
import json, sys, fnmatch, getpass, time, threading
...
def cp_api_call(api_call, api_call_parameters, session_ro=False) -> dict:
    ...
    username = input("Enter username or press <Enter> for API-KEY (MDS/SMS R80.40+): ")
    api_key = ""
    password = ""
    if sys.stdin.isatty():
        if username != "":
    ...
    with APIClient(client_args) as client:
        ...
        if api_key == "":
          ...
          login_res = client.login_with_api_key(api_key, read_only=session_ro)
        ...
        if login_res.success is False:
          ...
          if cp_api_version in cp_version:
            print(f"{bcolors.OKGREEN}  \_MDS Version: {cp_version[cp_api_version]}{bcolors.ENDC}")

        # Execute the API call and loop over all results pages
        print(f"{bcolors.OKGREEN}[+] API call execution in progress, patience grasshopper ...")
        total = -1
        dict_res = {}
...
def process_licensing(tmp_dict):
  ...
  try:
    ...
    print(f"{bcolors.FAIL}[-] Function: process_licensing - Failed parsing JSON file\n  \_{e}{bcolors.ENDC}")
    exit(1)
  # Instantiate totals
  mds_prim_total = 0
  mds_stand_total = 0
...
def banner():
 ...
 |_| \_|\____|____/|_|  |_| |_____|_|\___\___|_| |_|___/_|_| |_|\__, | 
  __  __ ____  ____     ______        __   ____                 |___/  
 |  \/  |  _ \/ ___|   / ___\ \      / /  / ___|___  _   _ _ __ | |_   
 | |\/| | | | \___ \  | |  _ \ \ /\ / /  | |   / _ \| | | | '_ \| __|  
 | |  | | |_| |___) | | |_| | \ V  V /   | |__| (_) | |_| | | | | |_   
 |_|  |_|____/|____/   \____|  \_/\_/     \____\___/ \__,_|_| |_|\__|  
...
def main():
  ...
  if file_path == "":
    ...
    try:
      ...
      print(f"{bcolors.FAIL}[-] Error reading file {file_path}\n{e}{bcolors.ENDC}")
      exit(1)
    except ValueError as e:
      print(f"{bcolors.FAIL}[-] Failed parsing JSON input file\n{e}{bcolors.ENDC}")
      exit(1)
